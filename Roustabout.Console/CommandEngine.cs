...
namespace Tharga.Toolkit.Console
{
    public class CommandEngine
    {
        ...
        private void ShowAssemblyInfo()
        {
            ...
            var titleattr = assembly.GetCustomAttributes<System.Reflection.AssemblyTitleAttribute>().FirstOrDefault();
            if(titleattr != null)
            {
                title = titleattr.Title;
            }

            if (assembly != null) _rootCommand.OutputInformationLine(string.Format("{0} (Version {1})", title, assembly.GetName().Version), _commandMode);
        }
